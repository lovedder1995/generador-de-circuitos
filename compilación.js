...
var walk_abort = /* @__PURE__ */ Symbol("abort walk");
...
function OutputStream(options) {
  ...
  var semicolon2 = options.beautify ? function() {
    print(";");
  } : function() {
    might_need_semicolon = true;
  };
  ...
  function append_comments(node, tail) {
    ...
    if (OUTPUT.length() > insert2) newline_insert = insert2;
  }
...
var nullish = /* @__PURE__ */ Symbol("This AST_Chain is nullish");
...
function inline_into_call(self2, compressor) {
  ...
  function return_value(stat2) {
    if (!stat2) return make_void_0(self2);
    if (stat2 instanceof AST_Return) {
      if (!stat2.value) return make_void_0(self2);
  ...
  function flatten_args(decls, expressions) {
    ...
    for (i = len; --i >= 0; ) {
      ...
      if (has_flag(name, UNUSED) || !name.name || scope.conflicting_def(name.name)) {
        ...
        if (!value && in_loop) value = make_void_0(self2);
...
var Compressor = class extends TreeWalker {
  ...
  in_32_bit_context(other_operand_must_be_number) {
    ...
    for (var i = 0, p; p = this.parent(i); i++) {
      ...
      if (p instanceof AST_Binary && // Don't talk about p.left. Can change branch taken
      (p.operator == "&&" && p.right === self2 || p.operator == "||" && p.right === self2 || p.operator == "??" && p.right === self2) || p instanceof AST_Conditional && p.condition !== self2 || p.tail_node() === self2) {
        self2 = p;
      } else {
        return false;
      }
