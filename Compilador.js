...
const Opciones_disponibles = {
    ...
    [ Compilar una vez ]
    */
    "Compilar una vez": () => { esbuild.build(Configuración).catch(() => process.exit(1) ) },
    /*
    ...
    */
    "Compilar y volver a compilar cada vez que se modifique un archivo que forme parte de la compilación": () => { esbuild.context(Configuración).then(contexto => { contexto.watch() } ).catch(() => process.exit(1) ) } }
...
/* El menú */ const elección = await prompts([
    /* nos permitirá */ { message: "Selecciona una opción para compilar:",
        ...
        /* entre las opciones disponibles: */ choices: Object.keys(Opciones_disponibles).map(opción => ({ title: opción, value: opción } ) ) } ] )
/*
...
--------------------------------------------
*/
/* Ejecutamos la opción elegida. */ if (elección.opción) { Opciones_disponibles[elección.opción]() }
